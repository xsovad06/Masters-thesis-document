...
When the aggregation went without problems, it is time to export the Unified Detection into single JSON file. This functionality is provided by the {\it export_unified_detection()} method which generates either a single frame format of or multi-frame output according to a program argument ({\it --single-frame-outpu}). The output format is shown in \in{Code}[code:unified-format-output]. Generally, the difference between the single and multi-frame output is that the multi-frame output has a dictionary on the top level, where the keys identifies individual frames(In image detections just one item with key \uv{frame_1}. The video detections contains more than one frame leading to multiple records in the dictionary.). On the other hand, the single-frame output has a List on the top level, where this list contains human detection instances.
