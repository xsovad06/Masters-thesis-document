...
%====================================================================%
% CTX-THESIS ver. 0.993 -- Thesis in ConTeXt                         %
%--------------------------------------------------------------------%
% Sazební styl pro ConTeXt -- (c) TH, 2013--2024                     %
% Typesetting style for ConTeXt                                      %
%--------------------------------------------------------------------%
% t. č. ve vývoji / under the development                            %
%====================================================================%
% 0.993 20240411 space between figurecomb and caption
% 0.992 20240314 ml+id added
...
% 0.986 20230412 CTX v LMTX difference with \par in macros
...
%   id --- Keiko Budhi Saputra (2024)
%   ml --- Sin Huei Ong (2024)
...
T.supportedlanguages = {cs=1,sk=1,en=1,ca=1,es=1,fi=1,fr=1,hy=1,id=1,ml=1,pt=1}
...
--inspect(lang,name) os.exit()
--inspect(languages) os.exit()
  ...
  if     not lang then lang="en" 
  elseif --not languages.data.variants[lang] then -- ctx("\\errmessage{The "..lang.." language has not been supported by ConTeXt yet.}") 
     not languages.data.labels.texts.chapter.labels[lang] then  ctx("\\errmessage{The "..lang.." language has not been supported by ConTeXt yet.}") 
  end
--if name=="keywords" then inspect(S) inspect(lang) io.read() 
--end
--inspect(name.."/"..lang.."/"..hash[name].labels[lang])
...
--if name=="keywords" then --inspect(T.setup) io.read() 
--inspect(name.."/"..T.setup.mainlanguage) 
--inspect(Lth.keywords)
--inspect( T.get_label (Lth,T.setup.mainlanguage,name)) 
--io.read()
--end
...
function T.setupthesis(keywords, keyvals)
  ...
  if not T.unilist[S.university] then T.unilist = require "./ctx-thesis-listofuni" 
  end
  ...
  S.declaration.ml="\\relax"
  S.declaration.id="\\relax"
...
\setupcaption[figurecomblt][spaceinbetween=-2.5ex]
...
\def\thesisKeywordsText{\ctxlua{
%inspect(S) io.read() 
ctx(get_label_thesis("keywords"))}}
...
\noindenting
  \def\pomAbstract##1##2##3##4##5##6##7{
     ...
     \par\blank[big]
%  \startluacode inspect(S) io.read() \stopluacode 
%  \startluacode inspect('##4') io.read() \stopluacode 
\noindenting##5
\stopframedtext%
  }
  {\startluacode 
  ...
  \startluacode S.mainlanguage=pomlang 
\stopluacode
  }
  {\startluacode 
	if S.mainlanguage=="enxxxx" 
...
%-- 	else ctx(S.mainlanguage) 
  \par\blank[3*big]\null\quad%
%  \startluacode inspect(S) os.exit() \stopluacode 
